      ...
      <div className="bg-black border border-zinc-800 rounded-2xl w-full max-w-6xl my-auto relative shadow-[0_0_100px_rgba(255,255,255,0.05)]">
        ...
        <div className="p-6 md:p-12">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-12">
            {[
              { id: SubscriptionTier.Free, name: "Guest", price: "Free", credits: "5/day" },
              { id: SubscriptionTier.Starter, name: "Starter", price: "$9", credits: "100" },
              { id: SubscriptionTier.Creator, name: "Creator", price: "$29", credits: "500", popular: true },
              { id: SubscriptionTier.Studio, name: "Studio", price: "$99", credits: "2000", premium: true }
            ].map((plan) => {
              const tierFeatures = TIER_FEATURES[plan.id];
              const isCurrent = plan.id === currentTier;

              return (
                <div key={plan.id} className={`flex flex-col p-6 rounded-xl border transition-all duration-300 h-full relative ${plan.popular ? 'bg-zinc-900/50 border-zinc-500 scale-105 z-10 shadow-2xl shadow-zinc-500/10' : plan.premium ? 'bg-white border-transparent' : 'bg-black border-zinc-800 hover:border-zinc-700'}`}>
                  {isCurrent && (
                    <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-white text-black text-[8px] font-black px-3 py-1 rounded-full uppercase tracking-tighter border border-zinc-200">Current Plan</div>
                  )}
          ...
          <div className="flex flex-col items-center gap-10">
            <a 
              ...
              className="px-12 py-4 bg-white text-black rounded-md text-[10px] font-black uppercase tracking-[0.2em] shadow-2xl hover:bg-zinc-200 transition-all flex items-center gap-3 active:scale-[0.98] no-underline"
            >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-8 border-t border-zinc-900 w-full">
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-zinc-500 leading-relaxed">Paid tier users receive dedicated support and early access to experimental features.</p>
                </div>
              </div>
              <div className="flex items-start gap-4">
                <div className="w-10 h-10 rounded-full bg-zinc-900 flex items-center justify-center shrink-0 border border-zinc-800">
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-zinc-500 leading-relaxed">Switch plans anytime via the Account Manager. Unused credits rollover while active.</p>
                </div>
              </div>
            </div>
          </div>
